...
def get_workbook():
    try:
        ...
        # IMPORTANT:
        # Make sure this EXACTLY matches your Google Sheet filename
        workbook = client.open("GlobalTrust_Inventory")
        ...
        1. Google Sheet is named EXACTLY:
           GlobalTrust_Inventory

        2. Share the Google Sheet with:
           globaltrust-inventory-access@globaltrustinventory.iam.gserviceaccount.com

        3. Enable BOTH:
           - Google Sheets API
           - Google Drive API

        4. Streamlit Secrets are correctly configured
        """)
...
try:
    ...
    st.error("❌ Worksheet tabs not found.")
    st.error(str(e))

    st.info("""
    Your Google Sheet must contain TWO tabs named EXACTLY:
...
with col1:
    st.markdown(
        '<div class="main-header">🎁 Global Trust Asset Management</div>',
        unsafe_allow_html=True
    )

    st.markdown(
        '<div class="sub-header">Client Gift & Swag Inventory Manager</div>',
        unsafe_allow_html=True
    )
        ...
        if not low.empty:
            ...
            for _, row in low.iterrows():
                ...
                label = (
                    ...
                    + (
                        f" ({row['Size/Variant']})"
                        if row["Size/Variant"] not in ["N/A", ""]
                        else ""
                    )
                )
                ...
                st.warning(
                    f"🔴 {label} — only "
                    f"{row['Quantity In Stock']} left "
                    f"(threshold: {row['Low Stock Threshold']})"
                )

        cat_filter = st.multiselect(
            "Filter by Category",
            df_inv["Category"].unique(),
            default=list(df_inv["Category"].unique())
        )

        filtered = df_inv[
            df_inv["Category"].isin(cat_filter)
        ]
    ...
    if df_inv.empty:
        ...
        if submitted:
            ...
            if not client_name or not given_by:
                ...
                st.error(
                    "Client name and advisor name are required."
                )
                ...
                if qty_give > current_qty:
                    ...
                    st.success(
                        f"✅ Gave {qty_give}x "
                        f"{selected_item} to {client_name}."
                    )
...
with tab3:
    ...
    if df_inv.empty:
        ...
        if submitted_r:
            ...
            current_r = int(
                df_inv.iloc[idx_r]["Quantity In Stock"]
            )
...
with tab4:
    ...
    if df_inv.empty:
        ...
        with a1:
            ...
            stock_summary = (
                df_inv.groupby("Item")["Quantity In Stock"]
                .sum()
                .reset_index()
            )
        ...
        with a2:
            ...
            val_by_cat = (
                df_inv.groupby("Category")["Total Value"]
                .sum()
                .reset_index()
            )
        ...
        if not df_log.empty:
            ...
            df_log["Quantity Given"] = pd.to_numeric(
                df_log["Quantity Given"],
                errors="coerce"
            ).fillna(0)

            given_summary = (
                df_log.groupby("Item")["Quantity Given"]
                .sum()
                .reset_index()
            )
